...
- select between fresh real frame and synthesized frame (when neither is ready, pass the most recent real frame and rely on the headset's LSR for rotational correction)
...
- validate graceful degradation to stale frames when synthesis is unavailable (the headset's LSR handles rotational correction)
